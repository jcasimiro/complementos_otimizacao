...
class Graph:
    ...
    def edmonds_karp(self, source, sink):
        ...
        while self.bfs(source, sink, parent):
            ...
            flow_matrix = [row.copy() for row in self.flow]
            flow_matrices.append(flow_matrix)

        return max_flow, flow_matrices
...
source = 0
sink = 14    
max_flow,flow_matrixes = g.edmonds_karp(source, sink)

# Get the residual graph
residual_graph = g.get_residual_graph()

# Print the adjacency matrix of the residual graph
...
# Print the flow matrices for each iteration
for i, matrix in enumerate(flow_matrixes):
    print(f"\nFlow Matrix at Iteration {i+1}:")
    for row in matrix:
        print(row)